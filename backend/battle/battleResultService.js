...
export async function settleBattleResult(room) {
  ...
  const settlement = await withMongoTransaction(async ({ session }) => {
    ...
    const [lifeADoc, lifeBDoc, seasonADoc, seasonBDoc] = await Promise.all([profiles.findOne({ userId: a.userId }, { session }), profiles.findOne({ userId: b.userId }, { session }), room.seasonKey ? seasonProfiles.findOne({ seasonKey: room.seasonKey, userId: a.userId }, { session }) : null, room.seasonKey ? seasonProfiles.findOne({ seasonKey: room.seasonKey, userId: b.userId }, { session }) : null]);
    ...
    const updates = [profiles.updateOne({ userId: a.userId }, { $set: { displayName: a.name, ...lifeStatsA, updatedAt: now }, $setOnInsert: { userId: a.userId, createdAt: now } }, { session, upsert: true }), profiles.updateOne({ userId: b.userId }, { $set: { displayName: b.name, ...lifeStatsB, updatedAt: now }, $setOnInsert: { userId: b.userId, createdAt: now } }, { session, upsert: true })];
    if (room.seasonKey && result.a !== "abandoned") updates.push(seasonProfiles.updateOne({ seasonKey: room.seasonKey, userId: a.userId }, { $set: { displayName: a.name, ...seasonStatsA, tier: tierA.name, updatedAt: now }, $setOnInsert: { seasonKey: room.seasonKey, userId: a.userId, createdAt: now } }, { session, upsert: true }), seasonProfiles.updateOne({ seasonKey: room.seasonKey, userId: b.userId }, { $set: { displayName: b.name, ...seasonStatsB, tier: tierB.name, updatedAt: now }, $setOnInsert: { seasonKey: room.seasonKey, userId: b.userId, createdAt: now } }, { session, upsert: true }));
