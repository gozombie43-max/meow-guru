...
Completed sessions remain immutable evidence, while `trainingSkillState`, `trainingReviewState` and `trainingQuestionExposure` are durable incremental learner state updated atomically when a session completes. Historical session-derived evidence is merged with durable state for upgrade compatibility. Dashboard reads no longer write `userSkillProfile`. Sessions completed by this release record `learningApplied` in the completion transaction; when the recent history is fully covered, durable skills are primary. Older history retains a compatibility replay using projected metadata, without question text, options, solutions, events or reserves. Review intervals are 1/3/7/21/60 days, reset by wrong, uncertain or slow performance; prior historical review stages are carried into the durable ledger on first post-upgrade recall. Correct guesses receive reduced mastery credit. Topic, subtopic and concept state are persisted. Taxonomy is learner-editable.
