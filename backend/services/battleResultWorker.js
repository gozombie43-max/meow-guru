...
export async function runBattleResultWorkerOnce() {
  ...
  try {
    ...
    const results = await Promise.allSettled(rooms.map((room) => processUnsettledBattleRoom(room)));
...
export async function startBattleResultWorker() {
  if (timer) return;
  await getBattleRoomsCollection().createIndex({ status: 1, resultRecordedAt: 1, finishedAt: 1 });
