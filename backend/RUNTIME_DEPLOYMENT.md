...
The Azure API workflow now requires its own build/tests and blocking secret/dependency scans, runs migrations, and checks worker heartbeats before deploying. After Azure OIDC login, it uses a `MONGODB_URI` GitHub Actions secret when configured and otherwise reads the existing `MONGODB_URI` setting from the `quizguru-backend` Web App without printing it. An optional `MONGODB_DB` repository variable or Web App setting selects the database and defaults to `quizDB`. The OIDC identity therefore needs permission to read Web App settings. Worker hosting and delivery remain environment-specific; the workflow deliberately fails if the matching worker release has not been deployed. Branch protection must require the relevant workflow checks separately.
