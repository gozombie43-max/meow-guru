...
The Azure API workflow now requires its own build/tests and blocking secret/dependency scans, runs migrations, and checks worker heartbeats before deploying. Configure its `MONGODB_URI` secret and optional `MONGODB_DB` variable. Worker hosting and delivery remain environment-specific; the workflow deliberately fails if the matching worker release has not been deployed. Branch protection must require the relevant workflow checks separately.
