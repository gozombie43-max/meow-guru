    ...
    return (
      <main className="adaptive-quiz-page">
        <div className="macos-window">
          ...
          {savedSession && (
            <section className="section-card" style={{ marginBottom: 16, borderColor: isDark ? 'rgba(0, 180, 160, 0.36)' : 'rgba(0, 180, 160, 0.32)'  }}>
              <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', justifyContent: 'space-between', gap: 12 }}>
                <div>
                  <div className="heading-title">Unfinished quiz saved</div>
                  <div className="subtext">Question {savedSession.currentIdx + 1} of {savedSession.questions.length} · {formatTime(savedSession.elapsed)} elapsed</div>
                </div>
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
                  <button type="button" onClick={resetQuiz} className="action-btn" style={{ padding: '9px 12px', background: 'transparent', color: 'var(--color-text-secondary)', border: '1px solid var(--color-border-tertiary)'  }}>Discard</button>
                  <button type="button" onClick={resumeSavedQuiz} className="action-btn" style={{ padding: '9px 12px', background: 'linear-gradient(135deg, #185FA5 0%, #00b4a0 120%)', color: '#fff'  }}>Resume test</button>
                </div>
              </div>
            </section>
          )}
          ...
          {error && (
            <div className="section-card" style={{ marginBottom: 18, background: isDark ? 'rgba(127, 29, 29, 0.20)' : 'rgba(254, 226, 226, 0.88)', borderColor: isDark ? 'rgba(248, 113, 113, 0.28)' : 'rgba(248, 113, 113, 0.40)'  }}>
          ...
          <div style={{ display: 'grid', gap: isMobileView ? 12 : 16 }}>
            <section className="section-card">
              <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 12, marginBottom: 16 }}>
                <div>
                  <div className="heading-title">Subjects</div>
                  <div className="subtext">Pick the mix you want the generator to optimize.</div>
                </div>
                <button onClick={() => setTopicsModalOpen(true)} className="action-btn" style={{ padding: isMobileView ? '10px 12px' : '12px 14px', background: isDark ? 'rgba(99, 102, 241, 0.18)' : '#eef2f7', color: 'var(--color-text-primary)', border: '1px solid var(--color-border-tertiary)'  }}>
                  Choose topics
                </button>
              </div>
            ...
            <section className="section-card">
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 16, marginBottom: 16 }}>
                <div>
                  ...
                  {!isMobileView && <div className="subtext">Control depth, pace, and runtime.</div>}
                </div>
            ...
            <section className="section-card">
              ...
              <div className="macos-segmented-control" style={{ gridTemplateColumns: 'repeat(2, minmax(0, 1fr))', display: 'grid', background: 'var(--seg-bg)' }}>
            ...
            <section className="section-card">
              <div style={{ display: 'flex', flexDirection: isMobileView ? 'column' : 'row', alignItems: 'center', justifyContent: 'space-between', gap: 12, marginBottom: 12 }}>
                <div className="macos-form-group" style={{ margin: 0, minWidth: 260 }}>
                  ...
                  <div style={{ position: 'fixed', inset: 0, zIndex: 50, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(2,6,23,0.55)' }}>
                    <div style={{ width: isMobileView ? '95%' : 'min(520px, 100%)', maxHeight: isMobileView ? '86vh' : '90vh', overflow: 'hidden', padding: isMobileView ? 10 : 20, ...topicsModalCardStyle, border: isDark ? '1px solid rgba(255,255,255,0.08)' : roundedCardStyle.border }}>
                      ...
                      <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 10, marginTop: 14, paddingTop: 12, borderTop: isDark ? '1px solid rgba(255,255,255,0.08)' : '1px solid var(--color-border-tertiary)', flexShrink: 0 }}>
                        <button type="button" onClick={() => setTopicsSelected([])} className="action-btn" style={{ padding: '8px 10px', background: 'transparent', border: '1px solid var(--color-border-tertiary)', color: 'var(--color-text-secondary)'  }}>Clear</button>
                        <button type="button" onClick={() => setTopicsModalOpen(false)} className="action-btn" style={{ padding: '8px 10px', background: 'transparent', border: '1px solid var(--color-border-tertiary)'  }}>Close</button>
                        <button type="button" onClick={() => setTopicsModalOpen(false)} className="action-btn" style={{ padding: '8px 10px', background: 'linear-gradient(135deg, #534ab7 0%, #00b4a0 120%)', color: '#fff'  }}>Apply topics</button>
                      </div>
                    </div>
                  </div>
                )}
    ...
    return (
      <main className="adaptive-quiz-page">
        <div className="macos-window">
          ...
          <section className="section-card" style={{ padding: isMobileView ? 12 : 14, marginBottom: 12  }}>
            <div style={{ display: 'flex', flexDirection: 'column', gap: isMobileView ? 10 : 12 }}>
              <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', justifyContent: 'space-between', gap: 12 }}>
                <div style={{ maxWidth: isMobileView ? '100%' : 680 }}>
                  ...
                  {!isMobileView && <p className="subtext" style={{ fontSize: 13, maxWidth: 720, margin: 0  }}>{meta.overallInsight}</p>}
                </div>
          ...
          <div style={{ display: 'grid', gap: 16 }}>
            <section className="section-card">
              <div style={{ marginBottom: 14 }}>
                <div className="heading-title">Today’s focus</div>
                <div className="subtext">The quiz will lean harder into the topics below.</div>
              </div>
            ...
            <section className="section-card">
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}>
                <button
                  ...
                  className="action-btn" style={{ minWidth: 210,
    ...
    return (
      <main className="adaptive-quiz-page">
        <div className="macos-window">
          ...
          <div className="section-card" style={{ padding: 18, marginBottom: 16, display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 14  }}>
            <div>
          ...
          <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: 12 }}>
            <button type="button" onClick={saveAndExit} className="action-btn" style={{ padding: '9px 12px', background: 'transparent', color: 'var(--color-text-secondary)', border: '1px solid var(--color-border-tertiary)'  }}>
              Save and exit
            </button>
          </div>
          ...
          <section className="section-card" style={{ padding: 20  }}>
            ...
            {submissionError && (
              <div role="alert" style={{ marginTop: 14, padding: 14, borderRadius: 14, background: 'var(--color-background-danger)', color: 'var(--color-text-danger)', border: '1px solid rgba(239, 68, 68, 0.28)', display: 'flex', flexWrap: 'wrap', alignItems: 'center', justifyContent: 'space-between', gap: 12 }}>
                ...
                <button type="button" onClick={submitQuiz} disabled={loading} className="action-btn" style={{ padding: '9px 12px', background: '#991b1b', color: '#fff', opacity: loading ? 0.65 : 1  }}>
    ...
    return (
      <main className="adaptive-quiz-page">
        <div className="macos-window">
          ...
          <section className="section-card" style={{ padding: isMobileView ? 14 : 22, marginBottom: 18  }}>
            <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', justifyContent: 'space-between', gap: isMobileView ? 12 : 16 }}>
              <div style={{ maxWidth: 640 }}>
                ...
                {!isMobileView && (
                  <p className="subtext" style={{ fontSize: 15  }}>The score below is paired with topic-level signals so you can see what to keep, what to review, and what to attack next.</p>
                )}
              </div>
          ...
          <div style={{ display: 'grid', gap: 16 }}>
            {topicAccuracy.length > 0 && (
              <section className="section-card">
                <div style={{ marginBottom: 14 }}>
                  <div className="heading-title">By topic</div>
                  <div className="subtext">Lower bars are your next revision targets.</div>
                </div>
            ...
            <section className="section-card">
              <div style={{ marginBottom: 14 }}>
                <div className="heading-title">Review</div>
                <div className="subtext">{results.filter((result) => !result.isCorrect).length} question{results.filter((result) => !result.isCorrect).length === 1 ? '' : 's'} still need attention.</div>
              </div>
            ...
            <section className="section-card">
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 12 }}>
                <button onClick={resetQuiz} className="action-btn" style={{ flex: '1 1 220px', background: 'transparent', color: 'var(--color-text-primary)', border: '1px solid var(--color-border-tertiary)'  }}>
                  New quiz
                </button>
                <button onClick={generateQuiz} className="action-btn" style={{ flex: '1 1 220px', background: 'linear-gradient(135deg, #534ab7 0%, #00b4a0 120%)', color: '#fff', boxShadow: '0 18px 34px rgba(83, 74, 183, 0.24)'  }}>
                  Retry same topics
                </button>
              </div>
            </section>
