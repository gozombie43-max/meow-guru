      ...
      <main className="mx-auto max-w-3xl px-3 pb-[110px] pt-3 sm:px-6 sm:pt-4">
        ...
        >
          <motion.div
            ...
            transition={{ duration: 0.2 }}
            className={`bg-white rounded-2xl shadow-[0_4px_20px_rgba(0,0,0,0.08)] px-6 py-6 sm:px-8 sm:py-8 mx-4 ${
          ...
          >
            <div className="flex items-center mb-[14px]">
              <span style={{
                ...
                fontWeight: 500,
              }}>{currentQ.concept}</span>
              <span style={{
                fontSize: '13px',
                color: '#6B7280',
                marginLeft: 8,
                fontWeight: 500,
              }}>{currentQ.exam} {currentQ.year}</span>
            </div>
            <div style={{ fontSize: 17, fontWeight: 500, color: '#111827', lineHeight: 1.65, marginBottom: 28 }}>
        ...
        <section className="mb-5">
          {currentQ.options.slice(0, 4).map((opt, i) => {
            // Option state logic
            let border = '#E5E7EB', bg = '#FFFFFF', letterBg = 'transparent', letterColor = '#1D4ED8', letterBorder = '#3B82F6', letterFontWeight = 600, letterText = '#1D4ED8', shadow = '0 1px 4px rgba(0,0,0,0.05)';
            let optionTextColor = '#111827';
            if (isCurrentSubmitted && i === currentQ.correctAnswer) {
              border = '#16A34A'; bg = '#F0FDF4'; letterBg = '#16A34A'; letterColor = '#fff'; letterBorder = '#16A34A'; letterText = '#fff';
            } else if (isCurrentSubmitted && selectedAnswer === i && i !== currentQ.correctAnswer) {
              border = '#DC2626'; bg = '#FEF2F2'; letterBg = '#DC2626'; letterColor = '#fff'; letterBorder = '#DC2626'; letterText = '#fff';
            } else if (!isCurrentSubmitted && selectedAnswer === i) {
              border = '#2563EB'; bg = '#EFF6FF'; letterBg = '#2563EB'; letterColor = '#fff'; letterBorder = '#2563EB'; letterText = '#fff';
            }
            return (
              <button
                key={i}
                ...
                onMouseOver={e => {
                  if (!isCurrentSubmitted && selectedAnswer !== i) {
                    e.currentTarget.style.border = '#93C5FD';
                    e.currentTarget.style.background = '#EFF6FF';
                  }
                }}
                onMouseOut={e => {
                  if (!isCurrentSubmitted && selectedAnswer !== i) {
                    e.currentTarget.style.border = '#E5E7EB';
              ...
              >
                ...
                {isCurrentSubmitted && i === currentQ.correctAnswer && (
                  <CheckCircle2 className="ml-auto h-5 w-5 shrink-0 text-emerald-600" aria-label="Right option" />
                )}
