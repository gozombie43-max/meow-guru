...
type MathematicsQuestionRecord = MathematicsQuestion & {
  quizName?: string;
  source?: string;
  quizId?: string;
};
...
const QUIZ_THEME_STORAGE_KEY = "mathematics-quiz-theme";
...
function syncQuizThemeToDom(
  ...
  const applyTheme = () => {
    const containers = document.querySelectorAll<HTMLElement>(".mathematics-quiz");
  ...
  const removeSwitchingClass = () => {
    const containers = document.querySelectorAll<HTMLElement>(
      ".mathematics-quiz.theme-switching"
    );
...
function toMathematicsQuestion(
  question: ApiQuestion,
  index: number,
  fallbackConcept: string
): MathematicsQuestionRecord {
...
function ensureUniqueQuestionIds(questions: MathematicsQuestionRecord[]): MathematicsQuestionRecord[] {
...
function SeriesConceptStart({
  ...
  const handleBack = () => {
    if (typeof window !== "undefined" && window.history.length > 2) {
      ...
      router.push(routeBase ?? `/mathematics/${slug}`);
    }
  };
...
function MathematicsQuizThemeStyles() {
  return (
    <style jsx global>{`
      .mathematics-quiz {
      ...
      .mathematics-quiz[data-theme="dark"] {
        color-scheme: dark;
      ...
      .mathematics-quiz .quiz-start {
        background: var(--quiz-start-bg);
        color: var(--quiz-text);
      }

      .mathematics-quiz .qstatus {
        border: 1px solid var(--quiz-status-empty-border);
      }
      .mathematics-quiz .qstatus--current {
      ...
      .mathematics-quiz .quiz-option {
        border-radius: 18px;
        box-shadow: var(--quiz-option-shadow);
      }
      .mathematics-quiz .quiz-option.is-selected {
        box-shadow: var(--quiz-option-selected-shadow);
      }
      .mathematics-quiz .quiz-option-letter {
        border-radius: 12px;
      }

      .mathematics-quiz .qnum-chip {
        border-radius: 12px;
      }

      .mathematics-quiz .quiz-icon-button {
        background: var(--quiz-surface);
        border: 1px solid var(--quiz-border);
        color: var(--quiz-text-muted);
      }
      .mathematics-quiz .quiz-icon-button:hover {
        background: var(--quiz-surface-muted);
      }

      .mathematics-quiz .quiz-bookmark:hover {
        background: var(--quiz-surface-muted);
      }

      .mathematics-quiz .theme-toggle {
        width: 34px;
        height: 34px;
        padding: 0;
        border-radius: 999px;
      ...
      .mathematics-quiz .theme-toggle:hover {
        transform: translateY(-1px);
        background: var(--quiz-toggle-bg);
        box-shadow: none;
      }
      .mathematics-quiz .theme-toggle:active {
        transform: translateY(0);
      }
      .mathematics-quiz .theme-toggle-icon {
        width: 19px;
        height: 19px;
        transition: color 0.2s ease, transform 0.2s ease;
      }
      .mathematics-quiz .theme-toggle:hover .theme-toggle-icon {
        transform: scale(1.05);
      }
      .mathematics-quiz .theme-toggle--light .theme-toggle-icon {
        color: #f59e0b;
      }
      .mathematics-quiz .theme-toggle--dark .theme-toggle-icon {
        color: #e2e8f0;
      }

      .mathematics-quiz[data-theme="dark"] .glass-card {
        background: rgba(15, 23, 42, 0.55);
        border: 1px solid rgba(148, 163, 184, 0.25);
        box-shadow: 0 10px 26px rgba(0, 0, 0, 0.35), inset 0 1px 0 rgba(255, 255, 255, 0.04);
      }
      .mathematics-quiz[data-theme="dark"] .glass-card:hover {
        box-shadow: 0 12px 30px rgba(0, 0, 0, 0.45);
        border-color: rgba(148, 163, 184, 0.35);
      }
      .mathematics-quiz[data-theme="dark"] .btn-outline {
        background: rgba(15, 23, 42, 0.4);
        border-color: rgba(148, 163, 184, 0.35);
        color: #e2e8f0;
      }
      .mathematics-quiz[data-theme="dark"] .btn-outline:hover {
        background: rgba(30, 41, 59, 0.6);
      }

      .mathematics-quiz.theme-switching .quiz-card,
      .mathematics-quiz.theme-switching .glass-card,
      .mathematics-quiz.theme-switching .quiz-option {
      ...
      .mathematics-quiz.theme-switching .quiz-start-button,
      .mathematics-quiz.theme-switching .quiz-start-button::after,
      .mathematics-quiz.theme-switching .quiz-start-icon {
...
export default function MathematicsQuizEngine({
  title,
  slug,
  routeBase,
  presentation = "default",
}: MathematicsQuizEngineProps) {
  ...
  const themeStyles = <MathematicsQuizThemeStyles />;

  const [allQuestions, setAllQuestions] = useState<MathematicsQuestionRecord[]>([]);
  const [questions, setQuestions] = useState<MathematicsQuestionRecord[]>([]);
  ...
  const storageKey = `mathematics_quiz_resume_${slug}_${mode}`;

  useEffect(() => {
    let active = true;
    const fallbackConcept = baseConcepts[0] ?? "General";

    fetchQuestions({ subject: "mathematics", topic: slug })
      .then((data) => {
        if (!active) return;
        setAllQuestions(
          ensureUniqueQuestionIds(
            data.map((item, index) => toMathematicsQuestion(item, index, fallbackConcept))
          )
        );
      })
      .catch((error) => {
  ...
  const quizKey = `mathematics:${slug}`;
  const quizHref = `${routeBase ?? `/mathematics/${slug}`}/quiz`;
  ...
  const currentQ = questions[currentIndex] as MathematicsQuestion | undefined;
  ...
  if (showAnalytics) {
    return (
      <div
        className="mathematics-quiz min-h-screen relative overflow-hidden"
        data-theme="light"
      ...
      >
        ...
        <div className="pt-28 pb-20 px-6 max-w-3xl mx-auto relative">
          ...
          >
            ...
            <Link
              href={routeBase ?? `/mathematics/${slug}`}
  ...
  if (!currentQ) {
    return (
      <div
        className="mathematics-quiz min-h-screen relative flex items-center justify-center"
        data-theme="light"
        style={{ background: "var(--quiz-bg)", color: "var(--quiz-text)" }}
      >
        {themeStyles}
  ...
  if (isMac) {
    return (
      <div className="mac-series-quiz mathematics-quiz" data-theme={theme}>
        {themeStyles}
  ...
  if (isIos) {
    return (
      <div className="ios-series-quiz mathematics-quiz" data-theme={theme}>
        {themeStyles}
  ...
  return (
    <div
      className="mathematics-quiz min-h-screen relative overflow-x-hidden"
      data-theme="light"
      style={{
