...
function buildDefaultFeatureCards(routeBase: string): FeatureCardData[] {
  return [
    {
      title: "PYQ",
    ...
    {
      title: "CareerWill",
      href: `${routeBase}/quiz?mode=formula`,
    ...
    {
      title: "Selection Way",
      href: `${routeBase}/quiz?mode=ai-challenge`,
...
export default function MathematicsTopicPage({
  title,
  slug,
  routeBase,
  eyebrow = "Mathematics",
  ...
  bannerActionLabel,
  bannerAriaLabel,
  bannerGradient = "linear-gradient(135deg, #ff7eb3 0%, #ff9f66 100%)",
  bannerIcon,
  featureTitle = "Explore Features",
  featureSubtitle = "Pick a mode and begin your personalized practice flow.",
  featureCards,
}: MathTopicPageProps) {
  ...
  const bannerVisual = bannerIcon ?? <DefaultBannerIcon />;
  ...
  return (
    <main className="math-topic-page">
      <div className="math-topic-container">
        <header className="math-topic-header">
          <p className="math-topic-eyebrow">{eyebrow}</p>
        ...
        <section className="feature-section">
          <h2>{featureTitle}</h2>
          <p className="feature-subtitle">{featureSubtitle}</p>

          <div className="feature-grid">
            {cards.map((card) => (
              <FeatureCard key={`${card.title}-${card.href}`} {...card} />
            ))}
          </div>
        </section>
      </div>
