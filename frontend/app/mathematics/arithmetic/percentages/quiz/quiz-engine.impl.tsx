...
import { type PercentageQuestion, PERCENTAGE_CONCEPTS } from "@/lib/percentages-questions";

// ── Types ────────────────────────────────────────────────────────────────────
type QuizMode = "concept" | "formula" | "mixed" | "ai-challenge" | "easy" | "hard";
type Difficulty = "easy" | "medium" | "hard";
type PercentageQuestionRecord = PercentageQuestion & {
  quizName?: string;
  source?: string;
  quizId?: string;
};
...
function toPercentageQuestion(question: ApiQuestion, index: number): PercentageQuestionRecord {
...
/* ── Concept Badge ───────────────────────────────────────────────────────────
 * Maps each percentages concept to a distinct accent colour so learners
...
export default function PercentagesQuizEngine() {
  ...
  // ── State ──────────────────────────────────────────────────────────────────
  const [allQuestions, setAllQuestions] = useState<PercentageQuestionRecord[]>([]);
  const [questions, setQuestions] = useState<PercentageQuestionRecord[]>([]);
  ...
  const quizTitle = "Percentages";
  const quizSlug = "percentages";
  const quizKey = `mathematics:${quizSlug}`;
  const quizHref = "/mathematics/arithmetic/percentages/quiz";
      ...
      <div className="min-h-screen relative overflow-hidden">
        <div className="pt-28 pb-20 px-6 max-w-3xl mx-auto relative">
          ...
          >
            ...
            <Link
              href="/mathematics/arithmetic/percentages"
