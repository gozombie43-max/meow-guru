...
type GeometryQuestionRecord = GeometryQuestion & {
  quizName?: string;
  source?: string;
  quizId?: string;
};
...
function toGeometryQuestion(question: ApiQuestion, index: number): GeometryQuestionRecord {
...
export default function TrigQuizEngine() {
  ...
  // ── State ──────────────────────────────────────────────────────────────────
  const [allQuestions, setAllQuestions] = useState<GeometryQuestionRecord[]>([]);
  const [questions, setQuestions] = useState<GeometryQuestionRecord[]>([]);
          ...
          {isImageQuestion ? (
            <div className="flex flex-col gap-3">
              <ImageMCQ
                key={currentQ.id}
                data={currentQ}
                onAnswer={handleSelectImageAnswer}
              />
              <button
                type="button"
                disabled={submitted}
