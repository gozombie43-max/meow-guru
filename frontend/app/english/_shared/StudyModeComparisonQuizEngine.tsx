...
function normalizeItems(value: unknown): StudyModeComparisonItem[] {
  ...
  return value.reduce<StudyModeComparisonItem[]>((items, item) => {
...
export default function StudyModeComparisonQuizEngine({
  config,
}: {
  config: StudyModeComparisonConfig;
}) {
    ...
    fetchQuestions({
      ...
      .then((data) => {
        ...
        const normalized = (Array.isArray(data) ? data : [])
          .map((entry, idx) =>
            toStudyModeCard(entry as unknown as StudyModeEntry, idx, config)
          )
  ...
  const activeCard = cards[currentPage - 1] || config.demoCard;
        ...
        <aside className="macos-sidebar">
          ...
          >
            <button
              type="button"
              className="light red"
              onClick={() => router.push(`/english/${config.topic}`)}
          ...
          <div className="dictionary-body-scroll">
            ...
            <section className="mobile-suite">
              <div className="mobile-seg-control">
                <button
                  type="button"
                  className={`m-tab ${mobileTab === "primary" ? "active m-syn" : ""}`}
                  onClick={() => setMobileTab("primary")}
                >
                  {config.primaryLabel} ({activeCard.primaryItems.length})
                </button>
                <button
                  type="button"
                  className={`m-tab ${mobileTab === "secondary" ? "active m-ant" : ""}`}
                  onClick={() => setMobileTab("secondary")}
                >
                  {config.secondaryLabel} ({activeCard.secondaryItems.length})
                </button>
              </div>
