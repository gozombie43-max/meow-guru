...
export type SectionConfig = {
  ...
  timeLimitMin: number;       // 0 = shares composite timer
  marking: { correct: number; incorrect: number };
  topics: string[];           // maps to Cosmos c.subject / c.topic
};

export type ExamConfig = {
  key: string;
  name: string;
  compositeTimer: boolean;    // true = one timer for whole paper, false = per-section
  totalDurationMin: number;
  sections: SectionConfig[];
};

export type MockTestSlot = {
  id: string;                 // unique, used in URLs: 'cgl-t1-mock-1'
  examSlug: string;           // routing: 'ssc-cgl'
  configKey: string;          // looks up ExamConfig: 'ssc-cgl-tier1'
  title: string;
  tier?: string;              // display label: 'Tier I' | 'Tier II' | 'CBT 1' | 'Prelims' etc.
  isFree: boolean;
  order: number;
  isPyq?: boolean;
  year?: number;
...
function generateSlots(
  ...
  const configName = EXAM_CONFIGS[configKey]?.name ?? examSlug;
...
export const MOCK_TEST_SLOTS: MockTestSlot[] = [
  // SSC
  ...
  ...generateSlots('ssc-mts', 'ssc-mts', undefined, 6, 2),
  ...generateSlots('ssc-gd', 'ssc-gd', undefined, 8, 2),

  // Railway
  ...generateSlots('rrb-ntpc', 'rrb-ntpc-cbt1', 'CBT 1', 8, 2),
  ...generateSlots('rrb-ntpc', 'rrb-ntpc-cbt2', 'CBT 2', 5, 1),
  ...generateSlots('rrb-group-d', 'rrb-group-d', undefined, 6, 2),
  ...
  // Management
  ...generateSlots('cat', 'cat', undefined, 6, 2),
  ...
  // University
  ...generateSlots('cuet', 'cuet', undefined, 6, 2),
...
export const PYQ_TEST_SLOTS: MockTestSlot[] = [
  // SSC
  ...
  ...generatePyqSlots('ssc-mts', 'ssc-mts', undefined, 5, 2),
  ...generatePyqSlots('ssc-gd', 'ssc-gd', undefined, 5, 2),

  // Railway
  ...generatePyqSlots('rrb-ntpc', 'rrb-ntpc-cbt1', 'CBT 1', 5, 2),
  ...generatePyqSlots('rrb-ntpc', 'rrb-ntpc-cbt2', 'CBT 2', 4, 1),
  ...generatePyqSlots('rrb-group-d', 'rrb-group-d', undefined, 5, 2),
  ...
  // Defence
  ...generatePyqSlots('nda', 'nda-math', 'Mathematics (Paper I)', 4, 1),
  ...generatePyqSlots('nda', 'nda-gat', 'General Ability (Paper II)', 4, 1),
  ...generatePyqSlots('cds', 'cds-english', 'English (Paper I)', 4, 1),
  ...generatePyqSlots('cds', 'cds-gk', 'General Knowledge (Paper II)', 4, 1),
  ...generatePyqSlots('cds', 'cds-math', 'Elementary Mathematics', 4, 1),

  // Management / University
  ...generatePyqSlots('cat', 'cat', undefined, 5, 2),
  ...generatePyqSlots('cuet', 'cuet', undefined, 5, 2),
];

// ─── Helpers ──────────────────────────────────────────────────────

export function getExamConfig(configKey: string): ExamConfig | undefined {
  return EXAM_CONFIGS[configKey];
}

export function getSlotById(slotId: string): MockTestSlot | undefined {
  return MOCK_TEST_SLOTS.find(s => s.id === slotId) || PYQ_TEST_SLOTS.find(s => s.id === slotId);
}

export function getSlotsForExam(examSlug: string): MockTestSlot[] {
  return MOCK_TEST_SLOTS.filter(s => s.examSlug === examSlug);
}

export function getPyqSlotsForExam(examSlug: string): MockTestSlot[] {
  return PYQ_TEST_SLOTS.filter(s => s.examSlug === examSlug);
}

export function getAllSlotsForExam(examSlug: string): MockTestSlot[] {
  return [
...
export function getTiersForExam(examSlug: string): string[] {
  const tiers = getAllSlotsForExam(examSlug)
    .filter(s => s.tier)
    .map(s => s.tier!);
  return [...new Set(tiers)];
}

/** Returns the right tier terminology for an exam category */
export function getTierLabel(examSlug: string): { singular: string; options: string[] } {
  const tiers = getTiersForExam(examSlug);
  if (tiers.length === 0) return { singular: '', options: [] };

  if (tiers[0]?.startsWith('Tier')) return { singular: 'Tier', options: tiers };
  if (tiers[0]?.startsWith('CBT'))  return { singular: 'Stage', options: tiers };
  if (tiers[0]?.startsWith('GS') || tiers[0]?.startsWith('CSAT') || tiers[0]?.startsWith('Math') || tiers[0]?.startsWith('General') || tiers[0]?.startsWith('English') || tiers[0]?.startsWith('Elementary'))
    return { singular: 'Paper', options: tiers };
  if (tiers[0]?.startsWith('Prelims') || tiers[0]?.startsWith('Mains'))
    return { singular: 'Phase', options: tiers };

  return { singular: 'Stage', options: tiers };
}

/** Total questions for an exam config */
export function getTotalQuestions(configKey: string): number {
  const config = EXAM_CONFIGS[configKey];
  if (!config) return 0;
  return config.sections.reduce((sum, s) => sum + s.questionCount, 0);
}

/** Total marks for an exam config */
export function getMaxMarks(configKey: string): number {
  const config = EXAM_CONFIGS[configKey];
  if (!config) return 0;
  return config.sections.reduce((sum, s) => sum + s.questionCount * s.marking.correct, 0);
}

// ─── Exam metadata (for catalog display) ──────────────────────────
