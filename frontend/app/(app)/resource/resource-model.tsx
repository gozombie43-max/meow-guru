import { BookOpen, Calculator, Globe2, Shapes } from "lucide-react";

export const resourceTabs = ["Books", "Chapter Wise", "Extra", "DPP"] as const;
...
export function getCategory(tab: ResourceTab) {
  if (tab === "Books") return "notes";
  if (tab === "Chapter Wise") return "formula";
  return tab.toLowerCase();
}

export function sortFiles(files: ResourceFile[]) {
...
export function formatSize(size?: number, fileName?: string) {
...
export const IosSpinner = ({ size = 32 }: { size?: number }) => (
  <svg
    width={size}
    height={size}
    viewBox="0 0 24 24"
