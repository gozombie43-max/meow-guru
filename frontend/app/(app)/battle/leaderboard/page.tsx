...
export default function BattleLeaderboardPage() {
  ...
  return <BattleSection title="Leaderboard" description="See who's setting the pace in the arena." loading={!data} error={Boolean(error)} retry={() => void mutate()}>
    <div className="bs-segments" role="group" aria-label="Ranking period">{(["season", "lifetime"] as const).map(value => <button key={value} aria-pressed={tab === value} onClick={() => setTab(value)}>{value === "season" ? "This season" : "All time"}</button>)}</div>
