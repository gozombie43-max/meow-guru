  ...
  return <div className="bs-page">
    ...
    <main className="bs-scroll-body">
      <div className="bs-content">
        ...
        {authLoading ? <Empty title="Preparing your arena…" loading /> : !user ? <Empty title="Sign in to see your battle activity" detail="Your rank, missions, and friends are linked to your account."><Link data-ui-button="secondary" className="bs-button" href="/login">Sign in</Link></Empty> : error ? <Empty title="Could not load this page" detail="Please try again. Your battle progress is saved."><button data-ui-button="secondary" className="bs-button" onClick={retry}>Try again</button></Empty> : loading ? <Empty title="Loading your battle activity…" loading /> : children}
      </div>
    </main>
  </div>;
}
