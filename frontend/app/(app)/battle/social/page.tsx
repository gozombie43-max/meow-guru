  ...
  return <BattleSection title="Social" description="Find your rivals. Bring your best." loading={!data} error={Boolean(error)} retry={() => void mutate()}>
    ...
    {data && <div className="bs-columns"><div className="bs-stack">
      ...
      <div>
        ...
        <div className="bs-segments" role="group" aria-label="Player groups">{(["friends", "favorites", "recentOpponents"] as const).map(value => <button data-ui-button="state" key={value} aria-pressed={tab === value} onClick={() => setTab(value)}>{value === "recentOpponents" ? "Recent" : value === "friends" ? "Friends" : "Favorites"}</button>)}</div>
