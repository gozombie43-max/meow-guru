...
function isFormulaQuestion(question: {
  ...
  return matchesQuizTag(question, ["careerwill", "factbank"]);
}
...
export default function GeneralAwarenessQuizEngine({
  ...
  const availableCount = useMemo(() => {
    let pool: GeneralAwarenessQuestionRecord[];
  ...
  useEffect(() => {
    ...
    fetchQuestions({ subject: "general-awareness", topic: slug })
      .then((data) => {
        ...
        setAllQuestions(
          data.map((item, index) =>
            toGeneralAwarenessQuestion(item, index, fallbackConcept)
          )
        );
      })
      .catch((error) => {
