  ...
  return (
    <div style={{
      ...
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 12 }}>
        <div>
          <h3 style={{ fontSize: 14, fontWeight: 500, margin: 0, color: "var(--color-text-primary)" }}>
            Mass Image Question Upload
          </h3>
          <p style={{ fontSize: 12, color: "var(--color-text-secondary)", margin: "4px 0 0", lineHeight: 1.5 }}>
            Upload a ZIP of question images. Each image patches the matching question ID and adds the image into its{" "}
            <code style={{ fontSize: 11, background: "var(--color-background-primary)", padding: "1px 5px", borderRadius: 4 }}>question</code>{" "}
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 10 }}>
        {[
          { value: "auto", label: "Auto (filename = questionId)", desc: "e.g. visual_1776252864088_9d03bca4.png patches that question ID" },
      ...
      <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
        ...
        >
          {uploading ? "Uploading..." : "Upload Question Images"}
        </button>
      </div>
      ...
      {response && (
        <div style={{ marginTop: 12 }}>
          <div style={{
            ...
            {response.success
              ? `✓ ${response.uploaded} question image${response.uploaded !== 1 ? "s" : ""} uploaded successfully`
              : "Upload failed"}
            {response.failed > 0 && (
